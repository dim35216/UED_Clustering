...
def GetRandomProjectionsMatrix(ushapeletCandidates, lenShapelet, numSubsequences):
    cardinality = 4
    dimensionality = 16
    shapeletsHash, shapeletsStorage = GetSaxHash(ushapeletCandidates, lenShapelet, cardinality, dimensionality)

    R = 10
    maskedNum = 3
    masks = np.zeros((R, maskedNum))
    A = ushapeletCandidates[:, 0]
    x = list(set(A))
    tsNumber = len(x)
    ...
    tsPerS = np.ones((numSubsequences, 1))
    vals = np.zeros((numSubsequences, 1))
    ...
    shValues = np.concatenate(np.array(list(shValues), dtype=object), axis=0).astype(np.int32)
    shStorageValues = np.concatenate(np.array(list(shStorageValues), dtype=object), axis=0).astype(np.int32)
